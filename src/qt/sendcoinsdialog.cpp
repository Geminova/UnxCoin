    ...
    model(0)
{
    ...
    ui->lineEditCoinControlChange->setPlaceholderText(
                      tr("Enter a UnxCoin address (e.g. UPg3KZKqkCHyVns744SDw6u1bKcnYDaLN3b)"));
    ...
    ui->comboCurrency->addItem(COLOR_NAME[UNX_COLOR_UNIH],
                               (int) UNX_COLOR_UNIH);
...
void SendCoinsDialog::setModel(WalletModel *model)
{
    ...
    if(model && model->getOptionsModel())
    {
        ...
        connect(model->getOptionsModel(), SIGNAL(displayUnitChangedUnxCoin(int)), this, SLOT(updateDisplayUnit()));
        connect(model->getOptionsModel(), SIGNAL(defaultColorChanged(int)), this, SLOT(currencyChanged(int)));

        // Coin Control
        connect(model->getOptionsModel(), SIGNAL(displayUnitChangedUnxCoin(int)), this, SLOT(coinControlUpdateLabels()));
        connect(model->getOptionsModel(), SIGNAL(coinControlFeaturesChanged(bool)), this, SLOT(coinControlFeatureChanged(bool)));
        connect(model->getOptionsModel(), SIGNAL(transactionFeeChangedUnxCoin(qint64)), this, SLOT(coinControlUpdateLabels()));
