...
#ifdef PREM_BUILD
    int nSubsidyColor = UNX_COLOR_PREM;
#else
    int nSubsidyColor = UNX_COLOR_UNIH;
#endif
...
#ifdef PREM_BUILD
    struct AMOUNT subsidy = GetPoWSubsidy(pindexBest, UNX_COLOR_PREM);
#else
    struct AMOUNT subsidy = GetPoWSubsidy(pindexBest, UNX_COLOR_UNIH);
#endif
...
Value getworkex(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "unxcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "unxcoin is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "unxcoin is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "unxcoin is downloading blocks...");
