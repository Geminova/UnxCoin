...
bool CWallet::LoadCScript(const CScript& redeemScript)
{
    ...
    if (redeemScript.size() > MAX_SCRIPT_ELEMENT_SIZE)
    {
        ...
        std::string strAddr = CBitcoinAddress(redeemScript.GetID(), UNX_COLOR_NONE).ToString();
...
std::pair<int, int64_t> CWallet::GetDebit(const CTxIn &txin) const
{
    ...
    return std::make_pair(UNX_COLOR_NONE, 0);
}
...

    {
        ...
        CTxDB txdb("r");
        {
            ...
            while (true)
            {
                ...
                // change
                //
                // unxcoin: sub-tenth of a cent change is moved to fee
                ...
                // fee delegate change
                //
                // unxcoin: sub-tenth of a cent fee change is moved to fee
...
bool CWallet::UnlockStealthAddresses(const CKeyingMaterial& vMasterKeyIn)
{
    ...
    for (; mi != mapCryptedKeys.end(); ++mi)
    {
        ...
        CBitcoinAddress addr(ckid, UNX_COLOR_NONE);
...
# if 0
        ...
        if (fDebug)
        {
            ...
            CBitcoinAddress coinAddress(keyID, UNX_COLOR_NONE);
                                  ...
                                  int64_t nFees[], CTransaction& txMint, CTransaction& txStake, CKey& key)
{
    ...
    // Calculate PoS reward
    {
        ...
        // unxcoin uses block rewards that do not depend on coin age
        uint64_t nCoinAge;
        CTxDB txdb("r");
...
bool CWallet::DelAddressBookName(const CTxDestination& address)
{
    ...
    NotifyAddressBookChanged(this, address, (int) UNX_COLOR_NONE, "", fOwned, CT_DELETED);
