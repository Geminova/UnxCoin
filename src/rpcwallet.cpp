...
Value getnewaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            ...
            "Returns a new unxcoin address for receiving payments.\n");
...
Value getaccountaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 1)
        throw runtime_error(
            ...
            "Returns the current unxcoin address for receiving payments to this account.");
...
Value setaccount(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 1 || params.size() > 2)
        throw runtime_error(
            "setaccount <unxcoinaddress> <account>\n"
...
Value sendtoaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 2 || params.size() > 7)
        throw runtime_error(
            "sendtoaddress <unxcoinaddress> <amount> [comment] [comment-to] [tx-comment] [product-id] [narration]\n"
...
Value signmessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 2)
        throw runtime_error(
            "signmessage <unxcoinaddress> <message>\n"
...
Value verifymessage(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 3)
        throw runtime_error(
            "verifymessage <unxcoinaddress> <signature> <message>\n"
...
Value sendfrom(const Array& params, bool fHelp)
{
    if (fHelp || params.size() < 3 || params.size() > 7)
        throw runtime_error(
            "sendfrom <fromaccount> <tounxcoinaddress> <amount> [minconf=1] [comment] [comment-to] [tx-comment] [product-id] [narration]\n"
            "<amount> is a real and is rounded to the nearest 0.000001\n"
            "[tx-comment] max size is " + strprintf("%d", MAX_TX_COMMENT_LEN) + "\n"
            "[product-id] is a positive integer\n"
	    "[narration] is a short (<=24 char) note"
...
Value encryptwallet(const Array& params, bool fHelp)
{
    ...
    StartShutdown();
    return "wallet encrypted; unxcoin server stopping, restart to run with encrypted wallet.  The keypool has been flushed, you need to make a new backup.";
}
...
Value validateaddress(const Array& params, bool fHelp)
{
    if (fHelp || params.size() > 2)
        throw runtime_error(
            "validateaddress <unxcoinaddress> [ticker]\n"
    ...
    CBitcoinAddress address(params[0].get_str());
    bool isValid;
    if ((nColor != (int) UNX_COLOR_NONE) && (address.nColor != nColor))
    {
       isValid = false;
    }
    else
...
Value validatepubkey(const Array& params, bool fHelp)
{
    ...
    if (isValid)
    {
        CTxDestination dest = address.Get();
        string currentAddress = address.ToString();
        if (address.nColor == UNX_COLOR_NONE)
        {
...
Value sendtostealthaddress(const Array& params, bool fHelp)
{
    ...
    {
        result.push_back(Pair("result", "Invalid UnxCoin Stealth Address."));
        return result;
    };
