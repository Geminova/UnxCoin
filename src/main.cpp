...
const string strMessageMagic = "UnxCoin Signed Message:\n";

// Settings

// nDefaultCurrency and nDefaultStake are for sideways compatibility.
int nDefaultCurrency = (int) UNX_COLOR_NONE;
...
int nDefaultStake = (int) UNX_COLOR_NONE;
...
struct AMOUNT GetPoWSubsidy(const CBlockIndex *pindexPrev, int nSubsidyColor)
{
    static const int64_t FIRST_DAY_MONEY = COIN[UNX_COLOR_UNIH] * 19110000;  // 19,110,000
    static const int64_t FIRST_DAY_SUBSIDY = 221180555556;  // 2211.80555556
    static const int64_t STANDARD_REWARD = 2187500000;  // 21.875

    static const int nLastFairLaunchBlock = GetLastFairLaunchBlock();
    static const int nLastFirstDayBlock = GetLastFirstDayBlock();
    static const int nLastPoWBlock = GetLastPoWBlock();

    struct AMOUNT stSubsidy;
    stSubsidy.nValue = 0;
    stSubsidy.nColor = (int) UNX_COLOR_UNIH;

    int nHeight = pindexPrev->nHeight + 1;

    if (nHeight <= nLastFairLaunchBlock)   // 360 = 60 * 6
    {
        stSubsidy.nValue = 0.1 * COIN[UNX_COLOR_UNIH];  // 10 * MIN_TXOUT_AMOUNT[UNX_COLOR_UNIH]
    }
    else if ((nSubsidyColor == UNX_COLOR_PREM) &&
             (pindexPrev->vMoneySupply[UNX_COLOR_PREM] < MAX_MONEY[UNX_COLOR_PREM]))
    {
       stSubsidy.nValue = 1;
       stSubsidy.nColor = (int) UNX_COLOR_PREM;
    }
    else if (nHeight < nLastFirstDayBlock)   // 360 + 1440 * 6 = 9000
    {
       stSubsidy.nValue = FIRST_DAY_SUBSIDY;   // FIRST_DAY_SUBSIDY = 2211 8055 5556
    }
    else if (nHeight == nLastFirstDayBlock)
    {
       // FIRST_DAY_MONEY = COIN[UNX_COLOR_UNIH] * 21 000 000
       stSubsidy.nValue = FIRST_DAY_MONEY - pindexPrev->vMoneySupply[UNX_COLOR_UNIH];
    }
    else if (nHeight <= nLastPoWBlock)  // 720 * 120 + 9000 = 86400 + 9000 = 95400
    {
        stSubsidy.nValue = STANDARD_REWARD;  // 21 8750 0000
    }
...
struct AMOUNT GetProofOfStakeReward(const CBlockIndex* pindexPrev)
{
    int nMintColor = (int) UNX_COLOR_UNIH;
    int nStakeColor = (int) UNX_COLOR_UNIH;
    ...
    int64_t nMaxPoSReward = (3 * (nSupply / 100)) / 262980;   // 3%, 262980 blocks/yr @120s

    stSubsidy.nValue = std::min(nMaxPoSReward, MAX_MONEY[nMintColor] - nSupply);
...
unsigned int UnxCoinGravityWave(const CBlockIndex* pindexLast, bool fProofOfStake)
{
...
{
    return UnxCoinGravityWave(pindexLast, fProofOfStake);
}
...
bool CBlock::ConnectBlock(CTxDB& txdb, CBlockIndex* pindex, bool fJustCheck)
{
    ...
    int nStakeColor = (int) UNX_COLOR_NONE;

    // *claimed* subsidy from GetProofOfStakeReward()
    int64_t nMintValue = 0;
    int nMintColor = (int) UNX_COLOR_NONE;
    ...
    if (IsProofOfWork())
    {
        ...
        pindex->nStakeColor = (int) UNX_COLOR_NONE;
    ...
    if (IsProofOfStake())
    {
        ...
        // unxcoin: reward based nHeight and color
        struct AMOUNT reward = GetProofOfStakeReward(pindex->pprev);

        // unxcoin: reward is earned in the mint transaction, not the stake transaction
...
bool CBlock::AcceptBlock()
{
    ...
    // UnxCoin: PoS does not start until PoW is finished
...
bool CheckDiskSpace(uint64_t nAdditionalBytes)
{
    ...
    if (nFreeBytesAvailable < nMinDiskSpace + nAdditionalBytes)
    {
        ...
        printf("*** %s\n", strMessage.c_str());
        uiInterface.ThreadSafeMessageBox(strMessage, "UnxCoin", CClientUIInterface::OK | CClientUIInterface::ICON_EXCLAMATION | CClientUIInterface::MODAL);
        StartShutdown();
        return false;
    }
    return true;
...
bool LoadBlockIndex(bool fAllowNew)
{
    ...
    if (mapBlockIndex.empty())
    {
        ...
        const char* pszTimestamp = "Revised Travel Ban Is Blocked Nationwide";
        CTransaction txNew;
        txNew.nVersion = 1;
        txNew.nTime = 1489638991;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.strTxComment = "";
        ...
        txNew.SetColor((int) UNX_COLOR_NONE);
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1489638991;
        ...
        assert(block.hashMerkleRoot ==
          uint256("0x4837e66fa70b794fdf828d5271336aa3e8ad22438752bbe9a74e78d301366f94"));
